...
class FME_Fieldsmanager_Model_Fieldsmanager extends Mage_Eav_Model_Entity_Attribute
{
	...
	foreach($data as $key=>$value){
	    ...
	    $attributeInfo = Mage::getResourceModel('eav/entity_attribute_collection')
			...
			->addFieldToFilter('entity_type_id',$this->getEntityTypeId())
                        ->getFirstItem();
	    $Entervalue= $this->getCurrentValue($mode,$key ,$attributeInfo,true);
	...
	return;
     ...
     public function getCurrentValue($session,$key ,$attributeInfo = null, $alldetails = false){
	$data = Mage::getSingleton($session . '/session')->getRegistry();
	...
	}elseif(Mage::app()->getStore()->isAdmin() && Mage::getSingleton('adminhtml/session_quote')->getStoreId()){ // fix to get the store id for new orders from backend
	    $newOrderCreateStoreId = Mage::getSingleton('adminhtml/session_quote')->getStoreId();
	   $collection->getSelect()->where('store_table.store_id ="'. $newOrderCreateStoreId . '" || store_table.store_id ="0"');
	}
